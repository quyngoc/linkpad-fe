...
import { fixedswapbsl } from "../interfaces";
...
class FixedSwapContractBsl {
	constructor({
		...
		try {
			...
			this.params = {
				...
				contract: new Contract(web3, fixedswapbsl, contractAddress),
			};
	...
	getPurchase = async ({ purchase_id }) => {
		...
		return {
			...
			wasFinalized: res[4],
			reverted: res[5],
			validAfterDate: Numbers.fromSmartContractTimeToMinutes(res[6]),
		};
	};
	...
	getBoughtAmount = async (address) => {
		const purchaseIds = await this.getAddressPurchaseIds({ address })
		const purchases = await Promise.all(purchaseIds.map(purchase_id => this.getPurchase({ purchase_id })))
		return purchases.reduce((pre, cur) => pre.addUnsafe(FixedNumber.from(cur.amount)), FixedNumber.from('0'))
	}
	...
	isBuyer = async (address) => {
		const ids = await this.getAddressPurchaseIds({ address })
		return !!ids.length
	}
	...
	getETHCostFromTokens = async ({ tokenAmount }) => {
		...
		// 	this.getDecimals()
		// );
		const amountWithDecimals = web3.utils.toWei(`${tokenAmount}`, 'ether')
		return Numbers.fromDecimals(
			await this.params.contract
				.getContract()
	...
	swap = async ({ tokenAmount, callback }) => {
		const amountWithDecimals = web3.utils.toWei(`${tokenAmount}`, 'ether')
		let ETHToWei = await this.getETHWeiCostFromTokens({
			tokenAmount,
		});
		return await this.__sendTx(
			this.params.contract.getContract().methods.buy(amountWithDecimals),
			false,
			ETHToWei,
			callback
		);
...
export default FixedSwapContractBsl;
