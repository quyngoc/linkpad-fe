...
import { fixedswapv4 } from "../interfaces";
...
class FixedSwapContractV4 {
	...
	getPurchase = async ({ purchase_id }) => {
		let res = await this.params.contract
			.getContract()
			.methods.getPurchase(walletStore.account, purchase_id)
			.call();
		return {
			_id: purchase_id,
			amount: Numbers.fromDecimals(res[0], this.getDecimals()),
			purchaser: res[1],
			ethAmount: Numbers.fromDecimals(res[2], this.tradeDecimals),
			validAfterDate: Numbers.fromSmartContractTimeToMinutes(res[3]),
			wasFinalized: res[4],
			percentage: res[5],
		};
	};
	...
	getPendingPurchase = async () => {
		let res = await this.params.contract
			.getContract()
			.methods.getPendingPurchase(walletStore.account)
			.call();
		return {
			_id: -1,
			amount: Numbers.fromDecimals(res[0], this.getDecimals()),
...
export default FixedSwapContractV4;
