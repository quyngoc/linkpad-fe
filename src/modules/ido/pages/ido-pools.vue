<template>
  <div>
        ...
        <div v-if="vm.exclusivePools.length">
          <div style="max-width: 500px;" class="mx-auto d-flex flex-column gap-2">
            ...
            <div
              ...
              @click="$router.push(`/pool/${poolStore.pool.slugName}`)"
            >
    ...
    <v-container class="py-16">
      <v-row>
        <v-col v-if="vm.hasFeaturedPools" cols="12" class="text-head3">On sale</v-col>
        <v-col cols="12" sm="6" md="4" v-for="model in vm.featuredPools" :key="model.pool.id">
          <featured-pool-item :model="model" />
        </v-col>
        <v-col cols="12"></v-col>
        <v-col v-if="vm.hasUpcommings" cols="12" class="text-head3">Upcoming Project</v-col>
        <v-col cols="12" md="4" v-for="model in vm.upcommingsPools" :key="model.pool.id">
          <upcoming-pool-item :model="model" />
        </v-col>
        <v-col cols="12"></v-col>
        <v-col v-if="vm.hasClosedPools" cols="6" class="text-h5 font-weight-bold">Closed Pools</v-col>
